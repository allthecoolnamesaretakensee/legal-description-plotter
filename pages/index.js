        ...
        {tieCoords && tieCoords.length > 1 && (
          <g>
            {tieCoords.slice(0, -1).map((coord, i) => {
              ...
              const bearing = tie ? 
                `${tie.quadrant?.charAt(0) || ''}${tie.degrees}°${tie.minutes}'${tie.quadrant?.charAt(1) || ''}`
                : '';
              const distance = tie ? `${(tie.distance_feet || 0).toFixed(2)}'` : '';
              
              // Calculate leader offset
              ...
              return (
                <g key={`tie-${i}`}>
                  ...
                  />
                  {/* Leader line */}
                  <line x1={midX} y1={midY} x2={labelX} y2={labelY} stroke="#06b6d4" strokeWidth="1" opacity="0.6" />
                  <circle cx={midX} cy={midY} r="2" fill="#06b6d4" />
                  
                  {/* Label box */}
                  <rect x={labelX - 30} y={labelY - 22} width="60" height="40" rx="3" fill="#0c1829" stroke="#06b6d4" strokeWidth="1" opacity="0.95" />
                  <text x={labelX} y={labelY - 10} textAnchor="middle" fill="#06b6d4" fontSize="9" fontWeight="bold">
                    T{i + 1}
                  </text>
                  <text x={labelX} y={labelY + 2} textAnchor="middle" fill="#e5e7eb" fontSize="7" fontFamily="monospace">
                    {bearing}
                  </text>
                  <text x={labelX} y={labelY + 12} textAnchor="middle" fill="#9ca3af" fontSize="7" fontFamily="monospace">
                    {distance}
                  </text>
                </g>
              );
        ...
        {showCombinedView && allParcels ? (
          ...
          <>
            ...
            <polygon
              ...
              strokeWidth="2"
            />
            {/* Line labels with leaders - L1, L2, etc with bearing/distance */}
            {coords.slice(0, -1).map((coord, i) => {
              ...
              // Calculate perpendicular offset for leader
              const dx = end.x - start.x;
              const dy = end.y - start.y;
              const len = Math.sqrt(dx*dx + dy*dy) || 1;
              
              // Alternate sides and vary distance to reduce overlap
              const offsetDir = (i % 2 === 0) ? 1 : -1;
              const offsetDist = 35 + (i % 3) * 10; // Vary between 35-55
              const perpX = (-dy / len) * offsetDist * offsetDir;
              const perpY = (dx / len) * offsetDist * offsetDir;
              ...
              // Get call data for this line
              const call = parcel.calls?.[i];
              const bearing = call ? 
                (call.call_type === 'curve' 
                  ? `C R=${call.radius}'`
                  : `${call.quadrant?.charAt(0) || ''}${call.degrees}°${call.minutes}'${call.quadrant?.charAt(1) || ''}`)
                : '';
              const distance = call ? 
              ...
              const lineColor = isErrorLine ? "#ef4444" : "#f59e0b";
              const textColor = isErrorLine ? "#fca5a5" : "#fbbf24";
              ...
              return (
                <g key={`label-${i}`}>
                  ...
                  <line 
                    ...
                    opacity="0.6"
                  />
                  {/* Small tick at line end */}
                  <circle cx={midX} cy={midY} r="2" fill={lineColor} />
                  
                  {/* Label background */}
                  <rect 
                    x={labelX - 28} y={labelY - 22} 
                    width="56" height="40" 
                    rx="3" 
                    fill="#111827" 
                    stroke={lineColor} 
                  ...
                  {/* Line number */}
                  <text x={labelX} y={labelY - 10} textAnchor="middle" fill={textColor} fontSize="9" fontWeight="bold">
                    L{i + 1}
                  </text>
                  {/* Bearing */}
                  <text x={labelX} y={labelY + 2} textAnchor="middle" fill="#e5e7eb" fontSize="7" fontFamily="monospace">
