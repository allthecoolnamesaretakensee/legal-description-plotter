        ...
        {showCombinedView && allParcels ? (
          ...
          <>
            ...
            {coords.slice(0, -1).map((coord, i) => {
              ...
              const offsetDir = dotProduct > 0 ? -1 : 1;
              
              const popupDist = 90;
              const popupX = midX + perpX * popupDist * offsetDir;
              const popupY = midY + perpY * popupDist * offsetDir;
              ...
              return (
                <g key={`line-${i}`}>
                  ...
                  {isCurve && call.radius ? (
                    ...
                    // The arc is determined by: radius, and whether it curves left or right
                    (() => {
                      const radius = call.radius * scale;
                      // Determine sweep: 'right' = clockwise = 1, 'left' = counter-clockwise = 0
                      // Default to right if not specified
                      const sweepFlag = (call.curve_direction === 'left') ? 0 : 1;
                  ...
                  {isSelected && (
                    <g>
                      ...
                      {isCurve ? (
                        ...
                        // Line info popup - wider for long text
                        <g transform={`translate(${popupX}, ${popupY})`}>
                          <rect x="-85" y="-45" width="170" height="90" rx="6" fill="#1f2937" stroke={lineColor} strokeWidth="2" />
                          <text x="0" y="-30" textAnchor="middle" fill="#fbbf24" fontSize="11" fontWeight="bold">LINE {i + 1}</text>
                          
                          <text x="-75" y="-12" fill="#9ca3af" fontSize="8">Bearing:</text>
                          <text x="75" y="-12" textAnchor="end" fill="#fff" fontSize="10" fontFamily="monospace">
                            {call?.quadrant?.charAt(0) || ''}{call?.degrees || 0}°{call?.minutes || 0}'{call?.seconds || 0}"{call?.quadrant?.charAt(1) || ''}
                          </text>
                          
                          <text x="-75" y="6" fill="#9ca3af" fontSize="8">Distance:</text>
                          <text x="75" y="6" textAnchor="end" fill="#fff" fontSize="10" fontFamily="monospace">
                            {(call?.distance_feet || 0).toFixed(2)}'
                          </text>
                          
                          {call?.along_description && (
                            <>
                              <line x1="-75" y1="16" x2="75" y2="16" stroke="#374151" strokeWidth="1" />
                              <text x="0" y="30" textAnchor="middle" fill="#6b7280" fontSize="7">
                                {call.along_description.length > 40 ? call.along_description.substring(0, 40) + '...' : call.along_description}
                              </text>
                            </>
                          )}
                        </g>
