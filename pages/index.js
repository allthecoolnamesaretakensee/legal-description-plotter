      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full bg-gray-950 rounded-lg border border-gray-700">
            ...
            {tieCoords.slice(0, -1).map((coord, i) => {
              ...
              return (
                <g key={`tie-${i}`}>
                  ...
                  <text x={midX} y={midY + 4} textAnchor="middle" fill="#06b6d4" fontSize="9" fontWeight="bold">
                    T{i + 1}
                  </text>
                </g>
              );
        ...
        {showReverse && errorZone && errorZone.largestGap && (
          <g>
            ...
            />
            {/* Error zone label */}
            <g transform={`translate(${(toSvg(errorZone.largestGap.forwardCoord).x + toSvg(errorZone.largestGap.reverseCoord).x) / 2}, ${(toSvg(errorZone.largestGap.forwardCoord).y + toSvg(errorZone.largestGap.reverseCoord).y) / 2 - 15})`}>
              <rect x="-35" y="-10" width="70" height="20" rx="4" fill="#7f1d1d" stroke="#ef4444" />
              <text x="0" y="4" textAnchor="middle" fill="#fca5a5" fontSize="9" fontWeight="bold">
                GAP: {errorZone.largestGap.distance.toFixed(1)}'
              </text>
            </g>
          </g>
        )}
        ...
        {showCombinedView && allParcels ? (
          ...
          <>
            ...
            <polygon
              ...
              strokeWidth="2"
            />
            {/* Line labels - L1, L2, etc with circles */}
            {coords.slice(0, -1).map((coord, i) => {
              ...
              // Highlight problem lines in red if in error zone
              ...
              return (
                <g key={`label-${i}`}>
                  <circle cx={midX} cy={midY} r="12" fill={isErrorLine ? "#7f1d1d" : "#1f2937"} stroke={isErrorLine ? "#ef4444" : "#f59e0b"} strokeWidth="1.5" />
                  <text x={midX} y={midY + 4} textAnchor="middle" fill={isErrorLine ? "#fca5a5" : "#fbbf24"} fontSize="9" fontWeight="bold">
                    L{i + 1}
                  </text>
                </g>
              );
        ...
        {/* Closure indicator */}
        {parcel.closure && (
          <g transform={`translate(${width - 100}, 12)`}>
            <rect x="0" y="0" width="90" height="26" rx="4" 
              fill={parcel.closure.closes ? "#065f4620" : "#7f1d1d20"} 
              stroke={parcel.closure.closes ? "#10b981" : "#ef4444"} />
            <text x="45" y="11" textAnchor="middle" fill={parcel.closure.closes ? "#10b981" : "#ef4444"} fontSize="9" fontWeight="bold">
              {parcel.closure.closes ? "CLOSES" : "NO CLOSE"}
            </text>
            <text x="45" y="21" textAnchor="middle" fill="#9ca3af" fontSize="8">
              {parcel.closure.precision_ratio}
            </text>
          </g>
        )}
              ...
              <div className="bg-gray-800 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 text-xs">
                  {result.parcels[activeParcel].calls?.map((call, i) => (
                    <div key={i} className="flex items-center gap-2 bg-gray-900/50 rounded px-2 py-1.5">
                      <span className="w-7 h-7 flex items-center justify-center rounded-full bg-amber-600/20 text-amber-400 font-bold text-xs">
                        L{i + 1}
                      </span>
                      <div className="flex-1 min-w-0">
                        <div className="text-gray-200 font-mono truncate">
                          {call.call_type === 'curve' 
                            ? `C: R=${call.radius}'` 
                            : `${call.quadrant?.charAt(0) || ''}${call.degrees}°${call.minutes}'${call.seconds || 0}"${call.quadrant?.charAt(1) || ''}`
                          }
                        </div>
                        <div className="text-gray-400">
                          {call.distance_feet || call.arc_length || call.chord_distance}'{call.distance_qualifier ? ' ±' : ''}
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                {result.tie_lines && result.tie_lines.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-gray-700">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 text-xs">
                      {result.tie_lines.map((tie, i) => (
                        <div key={i} className="flex items-center gap-2 bg-cyan-900/20 rounded px-2 py-1.5">
                          <span className="w-7 h-7 flex items-center justify-center rounded-full bg-cyan-600/20 text-cyan-400 font-bold text-xs">
                            T{i + 1}
                          </span>
                          <div className="flex-1 min-w-0">
                            <div className="text-gray-200 font-mono truncate">
                            ...
                            <div className="text-gray-400">{tie.distance_feet}'</div>
                          </div>
                        </div>
                      ))}
                    </div>
