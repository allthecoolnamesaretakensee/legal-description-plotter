...
export default async function handler(req, res) {
  ...
  try {
    const { parcels, rawText, originalLegal, filename } = req.body;
    
    // Use original legal text if provided, otherwise use rawText
    const legalText = originalLegal || rawText || "No legal description provided";
    ...
    // Raw Legal Description - use the original legal text
    children.push(
      new Paragraph({
    ...
    children.push(
      new Paragraph({
        spacing: { after: 300 },
        children: [new TextRun({ text: legalText, size: 22 })]
      })
    );
