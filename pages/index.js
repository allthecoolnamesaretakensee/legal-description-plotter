...
export default function Home() {
  ...
  const PlotView = ({ parcel, showCombinedView, allParcels }) => {
    ...
    // Include tie line coordinates in bounds calculation
    let allCoords = [...coords];
    if (tieCoords && tieCoords.length > 0) {
      allCoords = [...tieCoords, ...coords];
    }
    ...
    return (
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full bg-gray-950 rounded-lg border border-gray-700">
        ...
        {showCombinedView && allParcels ? (
          ...
          <>
            ...
            {coords.slice(0, -1).map((coord, i) => {
              ...
              return (
                <g key={`label-${i}`}>
                  <circle cx={midX} cy={midY} r="12" fill="#1f2937" stroke="#f59e0b" strokeWidth="1.5" />
                  <text x={midX} y={midY + 4} textAnchor="middle" fill="#fbbf24" fontSize="9" fontWeight="bold">
                    L{i + 1}
                  </text>
                </g>
              );
        ...
        {/* Legend for tie line */}
        {tieCoords && tieCoords.length > 0 && (
          <g transform={`translate(${width - 120}, ${height - 28})`}>
            <line x1="0" y1="8" x2="25" y2="8" stroke="#06b6d4" strokeWidth="2" strokeDasharray="6,3" />
            <text x="30" y="12" fill="#06b6d4" fontSize="8">Tie Line</text>
          </g>
        )}
      </svg>
    );
