...
function parseOneFractionalParcel(text, sectionWidth, sectionHeight, sectionInfo, parcelId) {
  ...
  const subdivisionHistory = [{ description: 'Section ' + (sectionInfo.section || '?'), bounds: Object.assign({}, bounds), width: sectionWidth, height: sectionHeight }];
  ...
  for (const part of parts) {
    ...
    console.log('After ' + part.original + ': ' + (bounds.maxX - bounds.minX) + ' x ' + (bounds.maxY - bounds.minY));
  }
  ...
  for (const less of lessOuts) {
    ...
    console.log('After LESS ' + dir + ' ' + dist + "': " + (bounds.maxX - bounds.minX) + ' x ' + (bounds.maxY - bounds.minY));
  }
