                  ...
                  {isSelected && (
                    <g>
                      ...
                      {isCurve ? (
                        ...
                        // Line info popup
                        <g transform={`translate(${popupX}, ${popupY})`}>
                          <rect x="-65" y="-40" width="130" height="80" rx="6" fill="#1f2937" stroke={lineColor} strokeWidth="2" />
                          <text x="0" y="-24" textAnchor="middle" fill="#fbbf24" fontSize="11" fontWeight="bold">LINE {i + 1}</text>
                          
                          <text x="-55" y="-6" fill="#9ca3af" fontSize="8">Bearing:</text>
                          <text x="55" y="-6" textAnchor="end" fill="#fff" fontSize="10" fontFamily="monospace">
                            {call?.quadrant?.charAt(0) || ''}{call?.degrees || 0}°{call?.minutes || 0}'{call?.seconds || 0}"{call?.quadrant?.charAt(1) || ''}
                          </text>
                          
                          <text x="-55" y="12" fill="#9ca3af" fontSize="8">Distance:</text>
                          <text x="55" y="12" textAnchor="end" fill="#fff" fontSize="10" fontFamily="monospace">
                            {(call?.distance_feet || 0).toFixed(2)}'
                          </text>
                          
                          {call?.along_description && (
                            <text x="0" y="30" textAnchor="middle" fill="#6b7280" fontSize="7">
                              {call.along_description.length > 30 ? call.along_description.substring(0, 30) + '...' : call.along_description}
                            </text>
                          )}
                        </g>
                      )}
      ...
      <div className="min-h-screen bg-gray-900 text-gray-100">
        ...
        <main className="max-w-6xl mx-auto p-4">
          ...
          {activeTab === 'results' && result && (
            <div className="space-y-4">
              {/* AUTO-FIXED BEARING ERRORS NOTIFICATION */}
              {result.parcels[activeParcel].bearing_errors?.length > 0 && (
                <div className="bg-gradient-to-r from-amber-900/40 to-yellow-900/40 border border-amber-500 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    <span className="text-2xl">🔧</span>
                    <div>
                      <h3 className="text-amber-400 font-bold">Bearing Errors Auto-Corrected</h3>
                      <p className="text-sm text-gray-300 mt-1">
                        The following impossible bearing values were automatically fixed:
                      </p>
                      <ul className="mt-2 space-y-1">
                        {result.parcels[activeParcel].bearing_errors.map((err, i) => (
                          <li key={i} className="text-sm font-mono">
                            <span className="text-amber-300">Call {err.callIndex + 1}</span>
                            <span className="text-gray-400"> ({err.type}): </span>
                            <span className="text-red-400">{err.original}'</span>
                            <span className="text-gray-400"> → </span>
                            <span className="text-green-400">{err.fixed}'</span>
                          </li>
                        ))}
                      </ul>
                    </div>
