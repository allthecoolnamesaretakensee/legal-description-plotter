  ...
  try {
    ...
    for (let p = 0; p < parcels.length; p++) {
      ...
      for (let i = 0; i < coords.length - 1; i++) {
        ...
        const midX = (curr.x + next.x) / 2;
        const midY = (curr.y + next.y) / 2;
        addText(lines, midX, midY + 2, `L${i + 1}`, 'TEXT', 3);
        ...
        if (calls[i]) {
          ...
          const label = `${bearing} ${dist.toFixed(2)}'`;
          ...
          addText(lines, midX + offsetX, midY + offsetY - 5, label, 'TEXT', 2.5);
        }
      }
      ...
      for (let i = 0; i < coords.length; i++) {
        ...
        addText(lines, c.x + 3, c.y + 3, label, 'TEXT', 2.5);
      }
    }
