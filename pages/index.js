...
export default function Home() {
  ...
  const PlotView = ({ parcel, showCombinedView, allParcels }) => {
    ...
    // Helper to format bearing for display
    const formatBearing = (call) => {
      if (!call) return '';
      if (call.call_type === 'curve') {
        return `R=${call.radius}'`;
      }
      const q = call.quadrant || '';
      return `${q.charAt(0)}${call.degrees}°${call.minutes}'${q.charAt(1) || ''}`;
    };
    
    const formatDistance = (call) => {
      if (!call) return '';
      const dist = call.distance_feet || call.arc_length || call.chord_distance;
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full bg-gray-950 rounded-lg border border-gray-700">
        ...
        {tieCoords && tieCoords.length > 1 && (
          <g>
                  ...
                  {call && (
                    <g>
                      <rect 
                        ...
                        opacity="0.8"
                      />
                      <text x={midX} y={midY - 8} textAnchor="middle" fill="#06b6d4" fontSize="9" fontFamily="monospace">
                        {formatBearing(call)} {formatDistance(call)}
                      </text>
                    </g>
                  )}
                </g>
            ...
            <g>
              ...
              <text x={toSvg(tieCoords[0]).x} y={toSvg(tieCoords[0]).y - 12} textAnchor="middle" fill="#06b6d4" fontSize="11" fontWeight="bold">
                POC
              </text>
            </g>
          </g>
        ...
        {showCombinedView && allParcels ? (
            ...
            return (
              <g key={pIdx}>
                <polygon
                  ...
                  strokeWidth="2"
                />
                {/* Draw bearing/distance labels on lines */}
                {p.coordinates.slice(0, -1).map((coord, i) => {
                  const start = toSvg(coord);
                  const end = toSvg(p.coordinates[i + 1]);
                  const midX = (start.x + end.x) / 2;
                  const midY = (start.y + end.y) / 2;
                  const call = p.coordinates[i + 1]?.call;
                  ...
                  return (
                    <g key={`label-${pIdx}-${i}`}>
                      <rect 
                        ...
                        opacity="0.85"
                      />
                      <text x={midX} y={midY + 3} textAnchor="middle" fill="#e5e7eb" fontSize="8" fontFamily="monospace">
                        {formatBearing(call)} {formatDistance(call)}
                      </text>
                    </g>
                  );
          ...
          <>
            ...
            {coords.slice(0, -1).map((coord, i) => {
              ...
              const midX = (start.x + end.x) / 2;
              const midY = (start.y + end.y) / 2;
              const call = coords[i + 1]?.call;
              
              if (!call) return null;
              
              // Calculate line angle for label positioning
              ...
              return (
                <g key={`label-${i}`}>
                  <rect 
                    x={midX + offsetX - 50} y={midY + offsetY - 8} 
                    width="100" height="14" 
                    fill="#111827" 
                    rx="2"
                    opacity="0.9"
                  />
                  <text x={midX + offsetX} y={midY + offsetY + 3} textAnchor="middle" fill="#fbbf24" fontSize="9" fontFamily="monospace">
                    {formatBearing(call)} {formatDistance(call)}
                  </text>
                </g>
              );
        ...
        {parcel.closure && (
          <g transform={`translate(${width - 110}, 15)`}>
            <rect x="0" y="0" width="100" height="28" rx="4" 
              fill={parcel.closure.closes ? "#065f4620" : "#7f1d1d20"} 
              stroke={parcel.closure.closes ? "#10b981" : "#ef4444"} />
            <text x="50" y="12" textAnchor="middle" fill={parcel.closure.closes ? "#10b981" : "#ef4444"} fontSize="10" fontWeight="bold">
              {parcel.closure.closes ? "CLOSES" : "NO CLOSE"}
            </text>
            <text x="50" y="23" textAnchor="middle" fill="#9ca3af" fontSize="8">
        ...
        {tieCoords && tieCoords.length > 0 && (
          <g transform={`translate(${width - 110}, ${height - 35})`}>
            <line x1="0" y1="10" x2="30" y2="10" stroke="#06b6d4" strokeWidth="2" strokeDasharray="6,3" />
            <text x="35" y="14" fill="#06b6d4" fontSize="9">Tie Line (POC→POB)</text>
          </g>
        )}
      </svg>
    );
