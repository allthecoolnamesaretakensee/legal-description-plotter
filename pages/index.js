...
export default function Home() {
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      // Reset zoom and pan for new legal
      setZoom(1);
            ...
            {tieCoords.slice(0, -1).map((coord, i) => {
              ...
              return (
                <g key={`tie-${i}`}>
                  ...
                  {isSelected && tie && (
                    <g>
                      <line x1={midX} y1={midY} x2={popupX} y2={popupY} stroke="#06b6d4" strokeWidth="1.5" strokeDasharray="4,2" />
        ...
        {showCombinedView && allParcels ? (
          ...
          <>
            ...
            {coords.slice(0, -1).map((coord, i) => {
              ...
              const centerX = coords.reduce((sum, c) => sum + toSvg(c).x, 0) / coords.length;
              const centerY = coords.reduce((sum, c) => sum + toSvg(c).y, 0) / coords.length;
              ...
              // Check if popup goes off top
              if (popupY - popupHeight < 0) {
                offsetDir = -offsetDir;
                popupX = midX + perpX * popupDist * offsetDir;
                popupY = midY + perpY * popupDist * offsetDir;
              }
              // Check if popup goes off bottom
              else if (popupY + popupHeight > svgHeight) {
                offsetDir = -offsetDir;
                popupX = midX + perpX * popupDist * offsetDir;
                popupY = midY + perpY * popupDist * offsetDir;
              }
              // Check if popup goes off left
              if (popupX - popupWidth < 0) {
                popupX = popupWidth + 10;
              }
              // Check if popup goes off right
              else if (popupX + popupWidth > svgWidth) {
              ...
              return (
                <g key={`line-${i}`}>
                  ...
                  {isSelected && (
                    <g>
                      {/* Leader line */}
                      <line x1={midX} y1={midY} x2={popupX} y2={popupY} stroke={lineColor} strokeWidth="1.5" strokeDasharray="4,2" />
                      
                      {/* Popup box */}
                      {isCurve ? (
                        // Curve info popup - comprehensive
                        <g transform={`translate(${popupX}, ${popupY})`}>
                          <rect x="-70" y="-55" width="140" height="110" rx="6" fill="#1e3a5f" stroke="#3b82f6" strokeWidth="2" />
                        ...
                        <g transform={`translate(${popupX}, ${popupY})`}>
                          <rect x="-120" y="-50" width="240" height={call?.along_description ? 145 : 85} rx="6" fill="#1f2937" stroke={lineColor} strokeWidth="2" />
