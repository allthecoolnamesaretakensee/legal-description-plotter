  ...
  try {
    ...
    if (tieLineCoordinates && tieLineCoordinates.length > 1) {
      for (let i = 0; i < tieLineCoordinates.length - 1; i++) {
        ...
        // Leader line
        addLine(lines, midX, midY, textX, textY, 'TIE_LINE');
        
        // Label at end of leader
        addText(lines, textX, textY + 4, `T${i + 1}`, 'TIE_LINE', 4);
        
        // Add bearing and distance if available
        if (tieLines && tieLines[i]) {
          const tie = tieLines[i];
          const bearing = formatBearing(tie);
          const dist = tie.distance_feet || 0;
          addText(lines, textX, textY, bearing, 'TIE_LINE', 3);
          addText(lines, textX, textY - 4, `${dist.toFixed(2)}'`, 'TIE_LINE', 3);
        }
      }
...
function formatBearing(call) {
  ...
  // Use 'd' for degrees instead of ° symbol to avoid CAD encoding issues
