            ...
            {coords.slice(0, -1).map((coord, i) => {
              ...
              return (
                <g key={`line-${i}`}>
                  ...
                  {/* Visible line */}
                  {isCurve && call.radius && call.curve_direction ? (
                    // Draw arc for curves
                    <path
                      d={`M ${start.x} ${start.y} A ${call.radius * scale} ${call.radius * scale} 0 0 ${call.curve_direction === 'right' ? 1 : 0} ${end.x} ${end.y}`}
                      fill="none"
                      stroke={isSelected ? "#60a5fa" : lineColor}
