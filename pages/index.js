    ...
    return (
      ...
      >
            ...
            {tieCoords.slice(0, -1).map((coord, i) => {
              ...
              return (
                <g key={`tie-${i}`}>
                  ...
                  />
                  <text x={midX} y={midY + 4} textAnchor="middle" fill={isSelected ? "#fff" : "#06b6d4"} fontSize="9" fontWeight="bold" style={{ pointerEvents: 'none' }}>
                    T{i + 1}
                  </text>
                  ...
                  {isSelected && tie && (
                    <g>
                      <line x1={midX} y1={midY} x2={popupX} y2={popupY} stroke="#06b6d4" strokeWidth="1.5" />
                      <circle cx={midX} cy={midY} r="3" fill="#06b6d4" />
                      
                      <g transform={`translate(${popupX}, ${popupY})`}>
                        <rect x="-55" y="-30" width="110" height="60" rx="4" fill="#0c1829" stroke="#06b6d4" strokeWidth="2" />
                        <text x="0" y="-15" textAnchor="middle" fill="#06b6d4" fontSize="10" fontWeight="bold">T{i + 1} - TIE LINE</text>
                        <text x="0" y="2" textAnchor="middle" fill="#fff" fontSize="9" fontFamily="monospace">
                          {tie.quadrant?.charAt(0) || ''}{tie.degrees || 0}°{tie.minutes || 0}'{tie.seconds || 0}"{tie.quadrant?.charAt(1) || ''}
                        </text>
                        <text x="0" y="18" textAnchor="middle" fill="#e5e7eb" fontSize="10" fontFamily="monospace">
        ...
        {showCombinedView && allParcels ? (
          ...
          <>
            {/* Forward path - draw lines and arcs */}
            {coords.slice(0, -1).map((coord, i) => {
              ...
              // For curves, try to draw actual arc
              if (isCurve && call.radius && call.curve_direction) {
                const radius = call.radius * scale;
                // Determine sweep direction: right curve = clockwise = 1, left = 0
            ...
            {coords.slice(0, -1).map((coord, i) => {
              ...
              // Determine which side to place popup (away from center)
              ...
              return (
                <g key={`label-${i}`}>
                  {/* Clickable circle */}
                  <circle 
                    cx={midX} cy={midY} r="13" 
                    fill={isSelected ? lineColor : "#1f2937"} 
                    stroke={lineColor} 
                    strokeWidth="2"
                    style={{ cursor: 'pointer' }}
                    onClick={(e) => {
                  ...
                  {isSelected && (
                    <g>
                      {/* Leader line */}
                      <line x1={midX} y1={midY} x2={popupX} y2={popupY} stroke={lineColor} strokeWidth="1.5" />
                      <circle cx={midX} cy={midY} r="3" fill={lineColor} />
                      
                      {/* Popup box */}
                      {isCurve ? (
                        // Curve info popup
                        <g transform={`translate(${popupX}, ${popupY})`}>
                          <rect x="-55" y="-50" width="110" height="100" rx="4" fill="#1e3a5f" stroke="#3b82f6" strokeWidth="2" />
                          <text x="0" y="-36" textAnchor="middle" fill="#93c5fd" fontSize="10" fontWeight="bold">C{i + 1} - CURVE</text>
                          <text x="0" y="-22" textAnchor="middle" fill="#fff" fontSize="8" fontFamily="monospace">R = {call.radius}'</text>
                          <text x="0" y="-10" textAnchor="middle" fill="#fff" fontSize="8" fontFamily="monospace">Δ = {call.central_angle_degrees || call.delta_degrees}°{call.central_angle_minutes || call.delta_minutes || 0}'{call.central_angle_seconds || call.delta_seconds || 0}"</text>
                          <text x="0" y="2" textAnchor="middle" fill="#fff" fontSize="8" fontFamily="monospace">Arc = {(call.arc_length || 0).toFixed(2)}'</text>
                          <text x="0" y="14" textAnchor="middle" fill="#9ca3af" fontSize="7">Chord:</text>
                          <text x="0" y="26" textAnchor="middle" fill="#fff" fontSize="8" fontFamily="monospace">{call.chord_quadrant?.charAt(0) || ''}{call.chord_degrees || 0}°{call.chord_minutes || 0}'{call.chord_seconds || 0}"{call.chord_quadrant?.charAt(1) || ''}</text>
                          <text x="0" y="38" textAnchor="middle" fill="#fff" fontSize="8" fontFamily="monospace">{(call.chord_distance || 0).toFixed(2)}'</text>
                        </g>
                      ) : (
                        // Line info popup
                        <g transform={`translate(${popupX}, ${popupY})`}>
                          <rect x="-50" y="-35" width="100" height="70" rx="4" fill="#1f2937" stroke={lineColor} strokeWidth="2" />
                          <text x="0" y="-20" textAnchor="middle" fill={textColor} fontSize="10" fontWeight="bold">L{i + 1}</text>
                          <text x="0" y="-4" textAnchor="middle" fill="#fff" fontSize="9" fontFamily="monospace">
                            {call?.quadrant?.charAt(0) || ''}{call?.degrees || 0}°{call?.minutes || 0}'{call?.seconds || 0}"{call?.quadrant?.charAt(1) || ''}
                          </text>
                          <text x="0" y="12" textAnchor="middle" fill="#e5e7eb" fontSize="10" fontFamily="monospace">
                            {(call?.distance_feet || 0).toFixed(2)}'
                          </text>
                          {call?.along_description && (
                            <text x="0" y="26" textAnchor="middle" fill="#9ca3af" fontSize="7">
                              {call.along_description.substring(0, 20)}...
                            </text>
                          )}
                        </g>
                      )}
