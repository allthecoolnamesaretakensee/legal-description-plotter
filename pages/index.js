...
export default function Home() {
  ...
  // Fix mouse wheel scrolling - attach event with passive: false
  useEffect(() => {
    const plotContainer = plotRef.current;
    if (!plotContainer) return;
    
    const handleWheelCapture = (e) => {
      // If the wheel event is on the SVG plot area, prevent page scroll
    ...
    plotContainer.addEventListener('wheel', handleWheelCapture, { passive: false });
    
    return () => {
      plotContainer.removeEventListener('wheel', handleWheelCapture);
    };
  }, [result]); // Re-attach when result changes
    ...
    return (
      <div className="relative">
        ...
        {showCombinedView && allParcels ? (
          ...
          <>
            {/* Fill polygon first (behind everything) */}
            <polygon
            ...
            {/* Draw lines - each line is clickable */}
                  ...
                  {isSelected && (
                    <g>
                      ...
                      {isCurve ? (
                        ...
                        >
                          <rect x="-70" y="-55" width="140" height="120" rx="6" fill="#1e3a5f" stroke="#3b82f6" strokeWidth="2" />
                          ...
                          {call.tangent_length && (
                            <>
                              <text x="-60" y="48" fill="#9ca3af" fontSize="8">Tangent:</text>
                              <text x="60" y="48" textAnchor="end" fill="#fff" fontSize="9" fontFamily="monospace">{call.tangent_length}'</text>
                            </>
                          )}
                          <text x="0" y="58" textAnchor="middle" fill="#4b5563" fontSize="7">drag to move</text>
                        </g>
                      ) : (
